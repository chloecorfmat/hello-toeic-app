<?php

use App\BadgeType;
use Illuminate\Database\Seeder;

class BadgeTypeSeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        /**BadgeType::create([
            'name' => 'Faire un challenge par jour, X jours d\'affilé',
            'description' => 'X dépend du niveau',
            'method' => 'aChallengePerDayXTimes'
        ]);

        BadgeType::create([
            'name' => 'Obtenir un score de X dans un challenge',
            'description' => 'X dépend du niveau',
            'method' => 'getXPointsOnChallengeMode'
        ]);

        BadgeType::create([
            'name' => 'Rester dans le top 10 des challenges X jours',
            'description' => 'X dépend du niveau',
            'method' => 'stayXDaysInChallengeTop'
        ]);

        BadgeType::create([
            'name' => 'Donner X bonnes réponses dans les challenges (tous confondus)',
            'description' => 'X dépend du niveau',
            'method' => 'getATotalOfXPointsInChallenge'
        ]);**/

        BadgeType::create([
            'name' => 'Valider X fois un test composé',
            'description' => 'X dépend du niveau',
            'method' => 'validateCompositeTestsXTimes'
        ]);

        BadgeType::create([
            'name' => 'Atteindre le top 10 des challenges X fois',
            'description' => 'X dépend du niveau',
            'method' => 'comeXTimesInChallengeTop'
        ]);

        BadgeType::create([
            'name' => 'Majorer une leçon X fois',
            'description' => 'X dépend du niveau',
            'method' => 'beFirstXTimesOnLesson'
        ]);

        BadgeType::create([
            'name' => 'Donner X bonnes réponses dans les exercices et tests composés (tous confondus)',
            'description' => 'X dépend du niveau',
            'method' => 'getXPointsInExercises'
        ]);



    }
}
